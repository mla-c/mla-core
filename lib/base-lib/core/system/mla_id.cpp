...
void mla_private_string_from_uint32_hex(mla_char_t *buffer, mla_uint32_t value, mla_size_t padding) {
    ...
    for (mla_size_t i = padding; i > 0; i--) {
        ...
        if (nibble < 10) {
            buffer[i - 1] = mla_s_cast<mla_char_t>('0' + nibble);
        } else {
            buffer[i - 1] = mla_s_cast<mla_char_t>('a' + (nibble - 10));
        }
        value >>= 4;
    }
}
...
mla_uint32_t mla_random_uint32() {
    ...
    if (!initialized) {
        if (sizeof(mla_platform_pointer_t) == 8) {
            ...
            mla_uint64_t addr1 = mla_r_cast<mla_uint64_t>(&seed);
            mla_uint64_t addr2 = mla_r_cast<mla_uint64_t>(&initialized);
            seed = mla_s_cast<mla_uint32_t>(addr1 ^ (addr1 >> 32) ^ addr2 ^ (addr2 >> 32));
        } else if (sizeof(mla_platform_pointer_t) == 4) {
            // 32-bit pointers: direct XOR
            mla_uint32_t addr1 = mla_s_cast<mla_uint32_t>(mla_r_cast<mla_uint64_t>(&seed));
            mla_uint32_t addr2 = mla_s_cast<mla_uint32_t>(mla_r_cast<mla_uint64_t>(&initialized));
            seed = addr1 ^ addr2;
        } else {
