...
mla_bytes_t mla_sha1(const mla_bytes_t &input) {
    ...
    mla_bytes_t padded_message = mla_bytes(mla_s_cast<mla_size_t>(padded_message_size));
    ...
    for (mla_uint8_t j = 0; j < 8; ++j) {
        padded[padded_message_size - 8 + j] =
            mla_s_cast<mla_byte_t>((bit_len >> (56 - (8 * j))) & 0xFF);
    }
    ...
    for (mla_uint64_t chunk = 0; chunk < padded_message_size / 64; ++chunk) {
        ...
        for (mla_uint32_t t = 0; t < 16; ++t) {
            mla_uint64_t idx = (chunk * 64ULL) + (mla_s_cast<mla_uint64_t>(t) * 4ULL);
            W[t] = (padded[idx] << 24) |
                   (padded[idx + 1] << 16) |
                   (padded[idx + 2] << 8) |
                   (padded[idx + 3]);
