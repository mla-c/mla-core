...
Tests are defined as header files (`.h`) that register test cases using the mla-c test framework (see [Test Support Library](../framework/core-test-support/readme.md)). Tests use `mla_test_t` definitions with assert macros for validation.
...
Tests are compiled and linked with the test support library and a platform-specific main entry point. See the [Test Support Library](../framework/core-test-support/readme.md) for framework details and the build system for compilation instructions.
