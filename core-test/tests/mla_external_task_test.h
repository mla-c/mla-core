...
#define mla_private_external_task_test_stdout_cmd "echo|set /p=hello"
...
#define mla_private_external_task_test_stdin_echo_cmd "findstr /R ."
#define mla_private_external_task_test_sleep_cmd "ping 127.0.0.1 -n 2 >nul"
#else
#define mla_private_external_task_test_stdout_cmd "printf 'hello'"
#define mla_private_external_task_test_stdin_echo_cmd "cat"
#define mla_private_external_task_test_sleep_cmd "sleep 1"
#endif

#define mla_private_external_task_test_default_timeout_ms 250
...
void ExternalTaskCreateAndReadStdOutTest() {
    ...
    mla_external_task_t task = mla_external_task_create(mla_string_const(mla_private_external_task_test_stdout_cmd));
    assert_false(mla_pointer_is_null(task.native_resource), "Task should be created");

    mla_byte_t buffer[8] = {0};
    mla_size_t read = mla_stream_input_read_with_timeout(task.std_out, 0, 5, buffer, mla_private_external_task_test_default_timeout_ms);
...
void ExternalTaskWriteStdInAndReadStdOutTest() {
    ...
    mla_external_task_t task = mla_external_task_create(mla_string_const(mla_private_external_task_test_stdin_echo_cmd));
    assert_false(mla_pointer_is_null(task.native_resource), "Task should be created");
    ...
    mla_byte_t buffer[8] = {0};
    mla_size_t read = mla_stream_input_read_with_timeout(task.std_out, 0, 4, buffer, mla_private_external_task_test_default_timeout_ms);
...
void ExternalTaskStateTest() {
    ...
    mla_external_task_t task = mla_external_task_create(mla_string_const(mla_private_external_task_test_sleep_cmd));
    assert_false(mla_pointer_is_null(task.native_resource), "Task should be created");
