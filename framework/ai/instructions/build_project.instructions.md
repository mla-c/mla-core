...
The project has no fixed main file and can be built with any build infrastructure (e.g., CMake, Zig, etc.). The repository contains a `framework/core/sources.cmake` file which lists the source files, but note that it uses a `lib/` prefix for paths which might not exist physically in the file system. The actual directories are `framework/core`, `framework/core-platform`, `core-test`, and `framework/core-test-support`.
