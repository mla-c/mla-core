...
The array list module (`framework/core/system/mla_array_list.h`) provides a generic, dynamically-resizing array. It is the standard ordered collection in the MLA framework — **never** use `std::vector`, raw arrays with `new[]`, or other standard containers.
