...
The serializer module (`framework/core/serializer/mla_serializer.h`) serializes and deserializes structured data to JSON, binary, or XML formats. Every struct that participates in serialization must implement two static member functions — `serialize` and `deserialize`.
