...
The RPC module (`framework/core/rpc/mla_rpc.h`) provides local and remote procedure call infrastructure. Procedures are identified by a string name, carry typed input/output structs, and are automatically discovered over HTTP when using `mla_rpc_http_server`.
