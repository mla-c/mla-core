...
The string module (`framework/core/system/mla_string.h`) is the primary text type in the MLA framework. `mla_string_t` supports UTF-8/16/32, small-string optimization (SSO up to 14 bytes), and reference-counted heap storage. **Never** use `std::string`, `char*` arithmetic, or standard library string functions directly.
