...
The task module (`framework/core/task/`) provides co-operative and native multi-tasking. Tasks are registered with the `mla_task_manager` and dispatched either in a single-threaded loop or via native OS threads, depending on the platform.
