...
The hash map module (`framework/core/system/mla_hash_map.h`) provides a template-based key-value store with automatic rehashing. It is the standard associative container in the MLA framework — **never** use `std::map`, `std::unordered_map`, or similar standard containers.
