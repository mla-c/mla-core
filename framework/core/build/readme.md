...
- **`framework/core/sources.cmake`**: Master source file list organizing all mla-c library components.
...
```bash
cmake -DCMAKE_C_COMPILER=framework/core/build/tools/zig/zig-cc.sh \
      -DCMAKE_CXX_COMPILER=framework/core/build/tools/zig/zig-cxx.sh ..
make
```

## Source Organization

The `framework/core/sources.cmake` file defines the complete set of source files organized by category:
...
Static analysis is integrated directly into `default-toolchain.cmake` via `clang-tidy.cmake`.  The shared check configuration lives in `framework/core/build/.clang-tidy` and is passed to clang-tidy by `clang-tidy.cmake`.
...
```cmake
include(lib/mla-core/framework/core/build/default-toolchain.cmake)
include(lib/mla-core/framework/core/build/default-compile-config.cmake)
...
The shared `.clang-tidy` file lives at `framework/core/build/.clang-tidy` and is passed explicitly by `clang-tidy.cmake` when analysis is enabled.
