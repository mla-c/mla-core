...
struct mla_task_shared_states {
    ...
    static mla_task_shared_states init() {
        return  {
            {TASK_STATE_UNKNOWN} // Initialize atomic int to 0 (TASK_STATE_STARTING)
        };
    }
};
...
struct mla_task_manager_low_level_access {
    ...
    mla_bool_t (*create_task)(const mla_task_worker_t worker, const mla_string_t& task_name, mla_user_data_t& user_data, const mla_task_stack_size stackSize, const mla_task_priority priority, mla_pointer_t& outTaskResourceOwner, const mla_pointer_t& shared_states);
    void (*run_tasks)();
