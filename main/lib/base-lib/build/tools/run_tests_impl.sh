...
if [ $build_exit_code -ne 0 ]; then
    echo "Error: Build phase failed with exit code $build_exit_code — aborting tests."
...
for suite in "${RUN_SUITES[@]}"; do
    ...
    echo "========================================================================"
    echo "Running Tests: $config_name"
    ...
    case "$runner_type" in
        native)
            "$full_binary_path" --test
            exit_code=$?
            ;;
        node)
            if [ -n "$NODE_BIN" ]; then
                "$NODE_BIN" "$full_binary_path" --test
                exit_code=$?
            else
        ...
        node_wasm_dir)
            if [ -n "$NODE_BIN" ]; then
                ...
                (cd "$binary_dir" && "$NODE_BIN" "$binary_name" --test)
                exit_code=$?
            else
                echo "Warning: Node.js not found, cannot run $binary_path — skipping."
                skipped_count=$((skipped_count + 1))
            ...
            ;;
        wasm)
            "$WORKSPACE_DIR/lib/base-lib/build/tools/run_wasm_standalone_test.sh" "$full_binary_path"
            exit_code=$?
            ;;
        none)
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        echo "Error: Tests FAILED for $config_name ($binary_path) — exit code $exit_code"
        failed_suites+=("$config_name ($binary_path)")
        failed_count=$((failed_count + 1))
    fi
done
