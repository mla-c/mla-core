...
static mla_bool_t __mla_xml_write_str(const mla_stream_output_t& out, const mla_char_t* str) {
    mla_size_t len = 0;
    while (str[len] != '\0') len++;
    if (len == 0) return true;
    return out.write(out, 0, len, reinterpret_cast<const mla_byte_t*>(str)) == len;
}

static mla_bool_t __mla_xml_write_escaped(const mla_stream_output_t& out, const mla_string_t& str) {
    for (mla_size_t i = 0; i < str.length; i++) {
        mla_char_t c = str.data[i];
        switch (c) {
            case '<': if (!__mla_xml_write_str(out, "&lt;")) return false; break;
            case '>': if (!__mla_xml_write_str(out, "&gt;")) return false; break;
            case '&': if (!__mla_xml_write_str(out, "&amp;")) return false; break;
            case '"': if (!__mla_xml_write_str(out, "&quot;")) return false; break;
            case '\'': if (!__mla_xml_write_str(out, "&apos;")) return false; break;
...
static mla_bool_t __mla_xml_close_tag_if_open(mla_serializer_t& inst) {
...
static mla_bool_t __mla_xml_write_attr(mla_serializer_t& inst, const mla_string_t& value) {
    ...
    if (!__mla_xml_write_str(inst.output, " ")) return false;
    if (!__mla_xml_write_escaped(inst.output, state->pending_property_name)) return false;
    if (!__mla_xml_write_str(inst.output, "=\"")) return false;
    if (!__mla_xml_write_escaped(inst.output, value)) return false;
    if (!__mla_xml_write_str(inst.output, "\"")) return false;
