//
// Created by chris on 1/26/2026.
//

#ifndef COREOS_MLA_UI_CONTROL_SURFACE_CONNECTOR_H
#define COREOS_MLA_UI_CONTROL_SURFACE_CONNECTOR_H

#include "../surfaces/mla_ui_surface.h"
#include "mla_ui_control.h"
#include "../../task/mla_mutx.h"

struct mla_ui_control_surface_connector_drawing_t {
    mla_array_list_t<mla_ui_surface_draw_command_t, mla_ui_surface_draw_command_initializer_t> drawCommands;
    mla_array_list_t<mla_ui_surface_input_event_t, mla_ui_surface_input_event_initializer_t> unprocessedInputEvents;
};

mla_ui_control_surface_connector_drawing_t mla_ui_control_surface_connector_drawing_empty();

struct mla_ui_control_surface_connector_rendering_t;

typedef mla_bool_t (*mla_ui_control_surface_connector_rendering_task_t)(mla_array_list_t<mla_ui_control_t, mla_ui_control_initializer_t> &root, const mla_ui_surface_size_t& surfaceSize, const mla_ui_surface_input_states_t &input_states);

struct mla_ui_control_surface_connector_rendering_t {
    mla_array_list_t<mla_ui_control_t, mla_ui_control_initializer_t> root;
    mla_array_list_t<mla_ui_control_input_area_t, mla_ui_control_input_area_initializer_t> inputAreas;
    mla_ui_control_surface_connector_rendering_task_t renderingTask;
};

mla_ui_control_surface_connector_rendering_t mla_ui_control_surface_connector_rendering_empty();

struct mla_ui_control_surface_connector_t {
    mla_string_t taskId;
    mla_buffer_reference_t task_cleanup;
    mla_ui_surface_t surface;
    mla_mutex_t lock;
    mla_ui_control_surface_connector_rendering_t rendering;
    mla_ui_control_surface_connector_drawing_t drawing;
};

mla_ui_control_surface_connector_t mla_ui_control_surface_connector_empty();

mla_ui_control_surface_connector_t mla_ui_control_surface_connector_create(const mla_ui_surface_t &surface);

mla_bool_t mla_ui_control_surface_connector_start(mla_ui_control_surface_connector_t &connector, mla_array_list_t<mla_ui_control_t, mla_ui_control_initializer_t>& root, const mla_ui_control_surface_connector_rendering_task_t &renderingTask);

mla_bool_t mla_ui_control_surface_connector_stop(mla_ui_control_surface_connector_t &connector);







#endif