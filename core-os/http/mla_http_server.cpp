...
mla_http_server_websocket_connection_t __mla_http_server_websocket_connection(mla_http_server_t& http_server, mla_network_connection_t& connection, const mla_http_server_websocket_handler_item_t& handlerItem, const mla_string_t& endpoint, mla_bool_t supports_deflate_compression) {
...
mla_bool_t __mla_http_server_request_read(mla_network_connection_t &connection, mla_http_request_t &request,
    ...
    if (mla_string_equals_const(transferEncoding, "chunked")) {
        ...
        if (mla_http_headers_has_header_value(request.headers, mla_string_const("Content-Encoding"), mla_string_const("deflate"), mla_string_const(","))) {
...
mla_task_process_result_state __mla_http_server_handler_new_request(mla_user_data_t& userData) {
    ...
    if (!processed) {
        ...
        for (mla_size_t i = 0; i < mla_array_list_size(copyWebsocketHandlers); i++) {
            ...
            mla_http_server_websocket_connection_t ws_connection = __mla_http_server_websocket_connection(server, clientConnection, handlerItem, request.url, supports_deflate_compression);
